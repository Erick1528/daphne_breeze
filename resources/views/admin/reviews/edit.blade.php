...
@section('content')
    <div class="mt-[80px] sm:mt-[120px]">
        <div class="mb-8 md:mb-10 flex items-center justify-between">
            <div>
                <h1 class="text-caribeCoffee text-2xl md:text-3xl font-bold">Editar Reseña</h1>
                <p class="mt-1 text-caribeCoffee/70 italic text-sm md:text-base">Modificar reseña de {{ $review->author_name }}</p>
            </div>
        ...
        <form action="{{ route('admin.reviews.update', $review) }}" method="POST" class="bg-white/80 border border-caribeCoffee/10 rounded-[16px] p-6 md:p-8 shadow-sm max-w-2xl">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="author_name" id="author_name" value="{{ old('author_name', $review->author_name) }}" required
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="4" required
                        class="w-full rounded-[12px] border border-caribeCoffee/20 bg-bgGray/30 px-4 py-2.5 text-caribeCoffee placeholder-caribeCoffee/40 text-sm md:text-base focus:border-caribeOrange/50 focus:outline-none focus:ring-2 focus:ring-caribeOrange/20">{{ old('content', $review->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="rating" id="rating" required
                        ...
                        @foreach([1,2,3,4,5] as $n)
                            <option value="{{ $n }}" {{ old('rating', $review->rating) == $n ? 'selected' : '' }}>{{ $n }} {{ $n == 1 ? 'estrella' : 'estrellas' }}</option>
                        @endforeach
                    </select>
                    @error('rating')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="date" name="review_date" id="review_date" value="{{ old('review_date', $review->review_date?->format('Y-m-d')) }}"
                        class="w-full rounded-[12px] border border-caribeCoffee/20 bg-bgGray/30 px-4 py-2.5 text-caribeCoffee text-sm md:text-base focus:border-caribeOrange/50 focus:outline-none focus:ring-2 focus:ring-caribeOrange/20">
                    @error('review_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="source" id="source"
                        ...
                        <option value="manual" {{ old('source', $review->source) === 'manual' ? 'selected' : '' }}>Manual</option>
                ...
                <div class="flex flex-wrap gap-6">
                    <div class="flex items-center gap-2">
                        ...
                        <input type="checkbox" name="active" id="active" value="1" {{ old('active', $review->active) ? 'checked' : '' }}
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="number" name="order" id="order" value="{{ old('order', $review->order) }}" min="0"
