<section id="reseñas" class="mt-[80px] sm:mt-[120px] w-full max-w-7xl mx-auto px-0 sm:px-6 scroll-mt-[100px]"
    ...
    @if ($reviews->isEmpty())
        <div class="rounded-[12px] border border-caribeCoffee/10 bg-white/60 p-6 md:p-8 text-center">
            <p class="text-caribeCoffee/70 text-sm md:text-base mb-5 md:mb-6">Próximamente podrás leer aquí las reseñas de nuestros huéspedes.</p>
    ...
    @else
        <div class="relative">
            <div class="overflow-x-auto snap-x snap-mandatory scroll-smooth flex gap-3 sm:gap-4 pb-4 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
                ...
                @foreach ($reviews as $review)
                    <article class="flex-shrink-0 w-full min-w-full snap-center rounded-[12px] border border-caribeCoffee/10 bg-white/60 p-6 md:p-8">
                        <div class="flex items-center gap-0.5 text-caribeOrange mb-3 md:mb-4" aria-label="{{ $review->rating }} de 5 estrellas">
                        ...
                        <p class="text-caribeCoffee/90 text-sm md:text-base leading-relaxed line-clamp-5">{{ $review->content }}</p>
                        <p class="mt-3 md:mt-4 font-semibold text-caribeCoffee text-sm md:text-base">{{ $review->author_name }}</p>
        ...
        <div class="mt-6 md:mt-8 text-center">
            <a href="https://maps.app.goo.gl/V5eBZf79WkEWj7Uk9" target="_blank" rel="noopener noreferrer" class="group inline-flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-caribeCoffee text-sm md:text-base hover:text-caribeOrange transition-colors">
